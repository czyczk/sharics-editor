...
import {AppSettings, EditorTheme} from '../../../../../shared/app-settings';
...
@Component({
  selector: 'app-editor-deprecated',
  templateUrl: './editor-deprecated.component.html',
  styleUrls: ['./editor-deprecated.component.scss']
})
export class EditorDeprecatedComponent implements OnInit, OnDestroy {
  ...
  insertTimestamp(removeExisting: boolean) {
    ...
    this.playbackService$.getTimestamp().subscribe(it => {
      ...
      // Select the word breaker if one is found
      if (wordBreakerNode) {
  ...
  removeTimestamp() {
    ...
    // Remove contents only if it's a timestamp
    const textContent = range.startContainer.textContent;
    if (textContent && textContent[0] === '[' && textContent[textContent.length - 1] === ']') {
